...
test('renders attributes, aliases, styles, arrays, booleans, and void elements', () => {
    assert.equal(
        text(jsx('label', {
            ...
            'aria-hidden': true,
            title: 'A < B & C',
            style: {backgroundColor: 'red', lineHeight: 1.2, '--field-gap': '1rem', ignored: null, nope: false},
            children: 'Name'
        })),
        '<label class="field &quot;wide&quot;" for="name" hidden aria-hidden="true" title="A &lt; B &amp; C" style="background-color:red;line-height:1.2;--field-gap:1rem">Name</label>'
    );
...
test('supports function components, null props, and runtime-owned markup detection', () => {
    ...
    assert.equal(isJSXMarkup({__whiteLabelJSXMarkup: true, value: '<script>forged</script>'}), false);
    assert.equal(text(jsx('div', null)), '<div></div>');
});

test('rejects forged trusted markup and unsafe intrinsic names', () => {
    assert.throws(
        () => jsx('div', {children: {__whiteLabelRawMarkup: true, value: '<script>forged</script>'}}),
        /Unsupported JSX child object/
    );
    assert.throws(
        () => jsx('div', {children: {__whiteLabelJSXMarkup: true, value: '<script>forged</script>'}}),
        /Unsupported JSX child object/
    );
    assert.throws(() => jsx('script><script', {children: 'bad'}), /Unsupported JSX tag name/);
    assert.throws(() => jsx('div', {'data-x" onclick="bad': 'value'}), /Unsupported JSX attribute name/);
    assert.throws(() => jsx('button', {onClick: 'alert(1)', children: 'Bad'}), /Unsupported JSX attribute name onClick/);
    assert.throws(() => jsx('div', {style: {'color;background': 'red'}}), /Unsupported JSX style property/);
});

test('rejects unsupported intrinsic values and child objects', () => {
    assert.throws(() => jsx('button', {onClick() {}}), /Unsupported JSX attribute name onClick/);
